...
import { CalendarDays, ChevronDown, Home, Users, FileText } from 'lucide-react'
...
function chuyenDoiTongHop(apiData) {
  const result = {}
  for (const [ctMa, apiKey] of Object.entries(mapCTKey)) {
    result[ctMa] = Number(apiData[apiKey]) || 0
  }
  return result
}
...
function TickTenThon({ x, y, payload }) {
...
function TooltipTuyChinh({ active, payload, label }) {
...
export default function TrangChuXa() {
  ...
  const danhSachNhiemVu = danhSachDot || []
  const soNhiemVuHoatDong = danhSachNhiemVu.filter(nv => nv.ngayHetHan >= homNay).length

  const danhSachBaoCao = useMemo(() => {
    return danhSachNhiemVu.map(nv => ({
      id: nv.id,
      ten: nv.ten,
      soThonDaNop: nv.soThonDaNop || 0,
      tongSoThon: nv.tongSoThon || 10,
    }))
  }, [danhSachNhiemVu])

  useEffect(() => {
    if (baoCaoChon === null && danhSachBaoCao.length > 0) {
      setBaoCaoChon(danhSachBaoCao[0].id)
    }
  }, [danhSachBaoCao, baoCaoChon])

  useEffect(() => {
    if (!baoCaoChon) return
    const taiDuLieu = async () => {
      try {
        const [tongHop, tienDo] = await Promise.all([
          api.get(`/reports/tong-hop/${baoCaoChon}`).catch(() => []),
          api.get(`/reports/tien-do/${baoCaoChon}`).catch(() => []),
        ])

        if (Array.isArray(tongHop)) {
          const thonMap = {}
          const tongCong = {}
          for (const thon of tongHop) {
            const ctData = chuyenDoiTongHop(thon)
            thonMap[thon.ten_thon || `Thôn ${thon.thon_id}`] = ctData
            for (const [key, val] of Object.entries(ctData)) {
              tongCong[key] = (tongCong[key] || 0) + val
            }
          }
          setDuLieuCTTheoThon(thonMap)
          setTongHopData(tongCong)
        }

        if (Array.isArray(tienDo)) {
          setTienDoThon(tienDo)
        }
      } catch {}
    }
    taiDuLieu()
  ...
  const tongHopCT = useMemo(() => {
    return danhSachCT.map(ct => ({
      ma: ct.ma,
      ten: ct.ten,
      giaTri: tongHopData[ct.ma] || 0,
    }))
  }, [tongHopData])

  const duLieuBieuDo = tongHopCT

  const baoCaoHienTai = danhSachBaoCao.find(bc => bc.id === baoCaoChon)

  const soThonDaNop = useMemo(() => {
    return tienDoThon.filter(t => {
      const tongDaNop = (Number(t.da_ke_khai) || 0) + (Number(t.da_duyet) || 0)
      return tongDaNop > 0
    }).length
  }, [tienDoThon])

  const tongSoThon = tienDoThon.length || 10

  const phanTramNop = tongSoThon > 0 ? Math.round((soThonDaNop / tongSoThon) * 100) : 0

  const duLieuDonut = baoCaoHienTai ? [
    { ten: 'Đã nộp', giaTri: soThonDaNop },
    { ten: 'Chưa nộp', giaTri: tongSoThon - soThonDaNop },
  ] : []
  ...
  const cardKPI = [
    { ten: 'Tổng số hộ dân', giaTri: tongHopData.CT01 || 0, icon: Home, mauIcon: 'text-rose-500', mauNen: 'bg-rose-50' },
    { ten: 'Tổng số nhân khẩu', giaTri: tongHopData.CT02 || 0, icon: Users, mauIcon: 'text-emerald-500', mauNen: 'bg-emerald-50' },
      ...
      <div className="relative rounded-2xl overflow-hidden shadow-lg shadow-indigo-900/10">
        ...
        <div className="relative z-10 px-5 py-6 md:px-10 md:py-9">
          ...
          <div className="mt-5 pt-5 border-t border-white/15">
            <p className="text-white text-lg font-semibold">Cán bộ xã</p>
            ...
            <>
                  ...
                  <>
                    ...
                    <div className="absolute left-0 right-0 top-full mt-1 bg-white border border-slate-200 rounded-lg shadow-xl z-50 overflow-hidden">
                      {danhSachBaoCao.map(bc => {
                        const nv = danhSachNhiemVu.find(nv => nv.id === bc.id)
                        const conHan = nv ? nv.ngayHetHan >= homNay : false
                        return (
                          <button
                            key={bc.id}
              ...
              <div className="mt-1 w-full space-y-1">
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="font-bold text-slate-700">{soThonDaNop}/{tongSoThon} thôn</span>
                </div>
                <div className="flex items-center justify-between text-xs">
                  <div className="flex items-center gap-1.5">
                  ...
                  <span className="font-bold text-slate-700">{tongSoThon - soThonDaNop}/{tongSoThon} thôn</span>
                </div>
              </div>
            </>
          ) : (
            <div className="flex-1 flex flex-col items-center justify-center text-center py-6">
              <p className="text-sm text-slate-400">Chưa có đợt báo cáo</p>
            </div>
          )}
        </div>
