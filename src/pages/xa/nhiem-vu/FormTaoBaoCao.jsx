...
export default function FormTaoBaoCao({ hienPopup, dongPopup, themBaoCao }) {
  ...
  const taoBaoCao = async () => {
    ...
    try {
      ...
      await themBaoCao(duLieu)
      dongPopup()
    } catch (err) {
      setLoiApi(err?.message || 'Không thể tạo đợt kê khai. Vui lòng thử lại.')
    } finally {
      setDangLuu(false)
    }
  }
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="px-6 py-4 border-t border-slate-200">
          ...
          <div className="flex items-center justify-end gap-3">
            <button onClick={dongPopup} disabled={dangLuu} className="px-5 py-2.5 text-sm font-semibold text-slate-600 hover:text-slate-800 hover:bg-slate-100 rounded-lg transition-colors disabled:opacity-50">
              Hủy
            </button>
            <button
              onClick={taoBaoCao}
              disabled={dangLuu}
              className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-blue-500 to-indigo-500 text-white font-semibold rounded-lg shadow-md shadow-indigo-500/25 transition-all duration-300 ease-out hover:-translate-y-1 hover:shadow-lg hover:shadow-indigo-500/30 active:translate-y-0 active:scale-[0.98] active:shadow-sm text-sm disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {dangLuu ? (
                <>
