...
import { CalendarDays, Clock, Users, ArrowLeft, CheckCircle2, FileText, Download, Database, BarChart3, Building2, FileSpreadsheet, Loader2 } from 'lucide-react'
...
export default function ChiTietBaoCao({ baoCao, danhSachThon, quayLai }) {
  ...
  const xuatExcel = useCallback(async () => {
    try {
      ...
      const res = await fetch(`${API_BASE}/reports/xuat-excel/${baoCao.id}`, {
        headers: { 'Authorization': `Bearer ${token}` },
      })
      if (!res.ok) throw new Error('Lỗi xuất file')
      const blob = await res.blob()
      const contentDisposition = res.headers.get('Content-Disposition')
      let tenFile = `BaoCao_${baoCao.ten}.xlsx`
      if (contentDisposition) {
        const match = contentDisposition.match(/filename="?(.+?)"?$/)
        if (match) tenFile = decodeURIComponent(match[1])
      }
