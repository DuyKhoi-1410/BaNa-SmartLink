...
import { CalendarDays, Home, Users, Clock, FileText, ChevronDown } from 'lucide-react'
import { PieChart, Pie, Cell, Tooltip, ResponsiveContainer } from 'recharts'
import { useNhiemVu } from '../../context/NhiemVuContext'
import { useAuth } from '../../context/AuthContext'
import { api } from '../../lib/api'
import { tinhThongKeHo, tinhTrangThai } from './nhiem-vu/utils'
...
const mapCTKey = {
  CT01: 'ct01_tong_ho',
  CT02: 'ct02_tong_nhan_khau',
  CT03: 'ct03_ho_ngheo',
  CT04: 'ct04_ho_can_ngheo',
  CT05: 'ct05_nguoi_co_cong',
  CT06: 'ct06_bao_tro_xh',
  CT07: 'ct07_tre_duoi_16',
  CT08: 'ct08_tre_hoan_canh',
  CT09: 'ct09_gia_dinh_van_hoa',
  CT10: 'ct10_tuoi_lao_dong',
  CT11: 'ct11_tham_gia_bhyt',
  CT12: 'ct12_thanh_vien_to_cnsc',
  CT13: 'ct13_huong_dan_dvc',
  CT14: 'ct14_bao_luc_gia_dinh',
}
...
export default function TrangChuThon() {
  ...
  const [thongKeThon, setThongKeThon] = useState({})
  const [duLieuThon, setDuLieuThon] = useState({
    CT01: 0, CT02: 0, CT03: 0, CT04: 0, CT05: 0, CT06: 0,
    CT07: 0, CT08: 0, CT09: 0, CT10: 0, CT11: 0, CT12: 0, CT13: 0, CT14: 0,
  })
  const [duLieuCTTheoDot, setDuLieuCTTheoDot] = useState({})

  const danhSachNhiemVu = layNhiemVuChoThon()

  const homNay = new Date().toISOString().slice(0, 10)
  const soNhiemVuHoatDong = danhSachNhiemVu.filter(nv => nv.ngayHetHan >= homNay).length

  useEffect(() => {
    if (!nguoiDung?.thon_id || danhSachNhiemVu.length === 0) return
    const taiThongKe = async () => {
      const ketQua = {}
      for (const nv of danhSachNhiemVu) {
        try {
          const tienDo = await api.get(`/reports/tien-do/${nv.id}?thon_id=${nguoiDung.thon_id}`)
          if (Array.isArray(tienDo) && tienDo.length > 0) {
            const t = tienDo[0]
            ketQua[nv.id] = {
              tongHo: t.tong_ho || 0,
              daKeKhai: t.da_ke_khai || 0,
              daDuyet: t.da_duyet || 0,
              traLai: t.tra_lai || 0,
            }
          }
        } catch {}
      }
      setThongKeThon(ketQua)
    }
    taiThongKe()
  }, [danhSachNhiemVu, nguoiDung?.thon_id])

  useEffect(() => {
    if (!nguoiDung?.thon_id || danhSachNhiemVu.length === 0) return
    const taiTongHop = async () => {
      const duLieuTheoDotMoi = {}
      let dotMoiNhat = null
      for (const nv of danhSachNhiemVu) {
        try {
          const data = await api.get(`/reports/tong-hop/${nv.id}?thon_id=${nguoiDung.thon_id}`)
          if (data) {
            const ctData = {}
            for (const [ctMa, apiKey] of Object.entries(mapCTKey)) {
              ctData[ctMa] = Number(data[apiKey]) || 0
            }
            duLieuTheoDotMoi[nv.id] = ctData
            if (!dotMoiNhat) dotMoiNhat = ctData
          }
        } catch {}
      }
      setDuLieuCTTheoDot(duLieuTheoDotMoi)
      if (dotMoiNhat) setDuLieuThon(dotMoiNhat)
    }
    taiTongHop()
  }, [danhSachNhiemVu, nguoiDung?.thon_id])
  ...
  const chiTietChoDuyet = useMemo(() => {
    return danhSachNhiemVu
      .filter(nv => nv.ngayHetHan >= homNay)
      .map(nv => {
        const tk = thongKeThon[nv.id]
        const choDuyet = tk ? tk.daKeKhai : 0
        return { ten: nv.ten, choDuyet }
      })
      .filter(item => item.choDuyet > 0)
  }, [danhSachNhiemVu, thongKeThon])

  const tongChoDuyet = chiTietChoDuyet.reduce((sum, item) => sum + item.choDuyet, 0)

  const tienDoNhiemVu = useMemo(() => {
    return danhSachNhiemVu.map(nv => {
      const tk = thongKeThon[nv.id] || { tongHo: 0, daKeKhai: 0, daDuyet: 0, traLai: 0 }
      const trangThai = tinhTrangThai(nv.ngayHetHan)
      const soNgayCon = Math.ceil((new Date(nv.ngayHetHan) - new Date(homNay)) / (1000 * 60 * 60 * 24))
      const choDuyet = tk.daKeKhai
      const daDuyet = tk.daDuyet
      const chuaNop = Math.max(0, tk.tongHo - choDuyet - daDuyet)
      return {
        id: nv.id,
        ten: nv.ten,
  ...
  const duLieuThayDoiCT = useMemo(() => {
    ...
    const ctHienTai = duLieuCTTheoDot[nvThayDoiId]
    if (!ctHienTai) {
      return danhSachCT.map(ct => ({ ma: ct.ma, ten: ct.ten, phanTram: 0 }))
    }
    const dotTruoc = danhSachNhiemVu.find((nv, idx) => {
      const nvIdx = danhSachNhiemVu.findIndex(n => n.id === nvThayDoiId)
      return idx === nvIdx + 1
    })
    const ctTruoc = dotTruoc ? duLieuCTTheoDot[dotTruoc.id] : null
    return danhSachCT.map(ct => {
      const valHienTai = ctHienTai[ct.ma] || 0
      if (!ctTruoc || ct.ma === 'CT01') {
        return { ma: ct.ma, ten: ct.ten, phanTram: 0 }
      }
      const valTruoc = ctTruoc[ct.ma] || 0
      const phanTram = valTruoc > 0
        ? Math.round(Math.abs(valHienTai - valTruoc) / valTruoc * 100)
        : (valHienTai > 0 ? 100 : 0)
      return { ma: ct.ma, ten: ct.ten, phanTram }
    })
  }, [nvThayDoiId, duLieuCTTheoDot, danhSachNhiemVu])
    ...
    <div className="max-w-7xl mx-auto px-3 py-4 md:px-6 md:py-8">
      ...
      <div className="relative rounded-2xl overflow-hidden shadow-lg shadow-indigo-900/10">
        ...
        <div className="relative z-10 px-5 py-6 md:px-10 md:py-9">
          ...
          <div className="mt-5 pt-5 border-t border-white/15">
            <p className="text-white text-lg font-semibold">Cán bộ thôn</p>
      ...
      <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:mt-8 md:gap-4">
        ...
        <div className="bg-rose-50 rounded-xl border border-slate-100 shadow-sm p-5 flex items-center gap-4 transition-all duration-300 ease-out hover:-translate-y-1 hover:shadow-md active:translate-y-0 active:scale-[0.98] active:shadow-sm">
          ...
          <div>
            <span className="text-2xl font-extrabold text-slate-800 block">{dinhDangSo(duLieuThon.CT01)}</span>
        ...
        <div className="bg-emerald-50 rounded-xl border border-slate-100 shadow-sm p-5 flex items-center gap-4 transition-all duration-300 ease-out hover:-translate-y-1 hover:shadow-md active:translate-y-0 active:scale-[0.98] active:shadow-sm">
          ...
          <div>
            <span className="text-2xl font-extrabold text-slate-800 block">{dinhDangSo(duLieuThon.CT02)}</span>
