...
const cauHinhLoai = {
  ...
  het_han: {
    icon: AlertTriangle,
    mauIcon: 'text-red-500',
    mauNen: 'bg-red-50',
    mauVien: 'border-red-200',
    nhan: 'Hết hạn',
    mauNhan: 'text-red-700 bg-red-100',
  },
}

function chuyenDoiThongBao(tb) {
  return {
    id: tb.id,
    loai: tb.loai || 'bao_cao_moi',
    tieuDe: tb.tieu_de || '',
    noiDung: tb.noi_dung || '',
...
export default function ThongBaoThon() {
  ...
  useEffect(() => {
    ...
    const taiThongBao = async () => {
      setDangTai(true)
      try {
        const data = await api.get(`/notifications?thon_id=${nguoiDung.thon_id}`)
        setDanhSach(Array.isArray(data) ? data.map(chuyenDoiThongBao) : [])
      } catch {}
      setDangTai(false)
    }
    taiThongBao()
  ...
  const xuLyClickThongBao = async (tb) => {
    if (!tb.daDoc) {
      try {
        await api.patch(`/notifications/${tb.id}/doc`)
      } catch {}
      setDanhSach(prev =>
        prev.map(item => item.id === tb.id ? { ...item, daDoc: true } : item)
      )
    }
    navigate('/thon/nhiem-vu', { state: { nhiemVuId: tb.nhiemVuId } })
  }

  const danhDauTatCaDaDoc = async () => {
    const chuaDoc = danhSach.filter(tb => !tb.daDoc)
    await Promise.all(chuaDoc.map(tb => api.patch(`/notifications/${tb.id}/doc`).catch(() => {})))
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5 md:mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs md:text-sm text-slate-500 mt-0.5">Thông báo từ xã và hệ thống</p>
          </div>
        </div>
      ...
      <div className="space-y-3">
        {danhSachHienThi.length > 0 ? (
          danhSachHienThi.map(tb => {
            const cauHinh = cauHinhLoai[tb.loai] || cauHinhLoai.bao_cao_moi
