...
export default function TrangDangNhap() {
  ...
  const xuLyBuoc1Dan = async (e) => {
    ...
    try {
      ...
      setLoiDan(err.error || 'Thông tin không khớp, vui lòng kiểm tra lại')
    } finally {
  ...
  const xuLyXacNhanOtpQuenMK = (e) => {
    e.preventDefault()
    if (!otpQuenMK.trim() || otpQuenMK.length !== 6) {
      setLoiDan('Vui lòng nhập mã OTP 6 số')
      return
    }
    setLoiDan('')
  ...
  const xuLyDangNhapMatKhauDan = async (e) => {
    ...
    try {
      ...
      setLoiDan(err.error || 'Mật khẩu không đúng')
    } finally {
      setDangXacThuc(false)
    }
  }
          ...
          <div className="bg-white/90 backdrop-blur-xl rounded-2xl md:rounded-3xl shadow-2xl shadow-black/20 p-5 md:p-8 border border-white/50">
            ...
            {vaiTro === 'dan' && (
              <div>
                ...
                {/* Bước 2: Chọn phương thức — OTP (mặc định) hoặc mật khẩu */}
                {buocDan === 2 && phuongThucDan === 'otp' && buocQuenMK === 0 && (
                  <div className="space-y-4">
                    ...
                    <div className="bg-blue-50 rounded-xl px-4 py-3 text-sm text-blue-700">
                      Xin chào <span className="font-bold">{hoTen}</span>. Mã OTP sẽ được gửi đến số điện thoại đăng ký <span className="font-bold">{sdtDaChe}</span> qua Zalo.
                    </div>
                    ...
                    >
                      {dangGuiOtp ? (
                        <><div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Đang gửi OTP...</>
                      ) : (
                ...
                {/* Quên mật khẩu — Bước 1: Đang gửi OTP */}
                {buocQuenMK === 1 && (
                  <div className="space-y-4">
                    <div className="flex items-center justify-center gap-3 py-8">
                      <div className="w-6 h-6 border-2 border-blue-300 border-t-blue-600 rounded-full animate-spin" />
                      <span className="text-sm text-slate-600">Đang gửi mã OTP qua Zalo...</span>
                    </div>
                  </div>
                )}
                ...
                {buocQuenMK === 2 && (
                  <form onSubmit={xuLyXacNhanOtpQuenMK} className="space-y-4">
                    <button type="button" onClick={() => { setBuocQuenMK(0); setLoiDan(''); setOtpQuenMK('') }} className="flex items-center gap-1.5 text-sm text-slate-500 hover:text-blue-600 transition-colors mb-1">
                      <ArrowLeft size={16} /> Quay lại đăng nhập
                    </button>
                    <div className="bg-amber-50 rounded-xl px-4 py-3 text-sm text-amber-700">
                      Mã OTP đã gửi qua <span className="font-bold">Zalo</span> đến số <span className="font-bold">{sdtDaChe}</span>. Nhập mã để đặt lại mật khẩu.
                    </div>
                    <div>
                    ...
                    <div className="text-center text-sm">
                      {demNguocQuenMK > 0 ? (
                        ...
                        <button
                          type="button"
                          onClick={() => { setDemNguocQuenMK(60); setOtpQuenMK('') }}
                ...
                {buocQuenMK === 3 && !doiMKThanhCong && (
                  <form onSubmit={xuLyDatMatKhauMoi} className="space-y-4">
                    <div className="mb-2">
                      ...
                      <p className="text-slate-500 text-sm mt-0.5">Nhập mật khẩu mới cho tài khoản của bạn</p>
                    </div>
                    <div>
                ...
                {buocDan === 3 && buocQuenMK === 0 && (
                  <form onSubmit={xuLyBuoc3Dan} className="space-y-4">
