...
export default function ChinhSuaThongTin() {
  ...
  const [form, setForm] = useState({
    ...
    diaChi: `${nguoiDung?.ten_thon || ''}, xã Bà Nà, huyện Hòa Vang, TP. Đà Nẵng`,
    thon: nguoiDung?.ten_thon || '',
  })
      ...
      <form onSubmit={xuLyLuu} className="p-4 md:p-8">
        <div className="space-y-6">
          ...
          <div>
            ...
            <textarea
              value={form.diaChi}
              onChange={(e) => xuLyThayDoi('diaChi', e.target.value)}
              rows={3}
              className="w-full px-4 py-3 rounded-xl border border-slate-300 text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all resize-none"
              placeholder="Nhập địa chỉ"
            />
          </div>

          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">Thôn</label>
            <select
              value={form.thon}
              onChange={(e) => xuLyThayDoi('thon', e.target.value)}
              className="w-full px-4 py-3 rounded-xl border border-slate-300 text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all bg-white"
            >
              <option>Thôn Phú Hoà</option>
              <option>Thôn Thái Lai</option>
              <option>Thôn Phước Khương</option>
              <option>Thôn Hoà Nhơn</option>
              <option>Thôn Sơn Phước</option>
              <option>Thôn Hoà Ninh</option>
              <option>Thôn An Sơn</option>
              <option>Thôn Thạch Nham Đông</option>
              <option>Thôn Thạch Nham Tây</option>
              <option>Thôn Phước Hưng</option>
            </select>
          </div>
        </div>
