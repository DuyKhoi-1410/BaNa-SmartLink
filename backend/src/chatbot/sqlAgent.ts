...
import { GENERATION_MODEL, GENERATION_TEMPERATURE } from './config.js'
...
9. Trả về TỐI ĐA 50 dòng (LIMIT 50).
...
export async function askData(question: string): Promise<AskDataResult> {
  const genSqlPrompt = `${SYSTEM_PROMPT}\n\n## Câu hỏi của cán bộ xã:\n${question}`

  const rawSql = await geminiGenerate(genSqlPrompt)
...
## Trả lời:`
  ...
  const answer = await geminiGenerate(summarizePrompt) || 'Không thể tóm tắt kết quả.'
