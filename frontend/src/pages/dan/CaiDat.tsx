...
import { User, Edit3, Lock, Monitor, FileText, HelpCircle, BookOpen, Camera, ChevronDown } from 'lucide-react'
...
export default function CaiDat() {
  ...
  const laTrangGoc = location.pathname === '/dan/cai-dat' || location.pathname === '/dan/cai-dat/'
  const [moSidebar, setMoSidebar] = useState(false)
  const tkRef = useRef(null)
  const htRef = useRef(null)
  const [tkSlider, setTkSlider] = useState<any>({})
  const [htSlider, setHtSlider] = useState<any>({})
        ...
        <div className={`${moSidebar ? 'block' : 'hidden'} md:block w-full md:w-72 md:flex-shrink-0`}>
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <div className="flex flex-col items-center -mt-10 md:-mt-12 pb-5 md:pb-6 px-5">
              <div className="relative">
                <div className="w-20 h-20 md:w-24 md:h-24 bg-white rounded-full border-4 border-white shadow-md flex items-center justify-center">
                  <div className="w-16 h-16 md:w-20 md:h-20 bg-gradient-to-br from-blue-400 to-indigo-500 rounded-full flex items-center justify-center">
                    <span className="text-white text-xl md:text-2xl font-bold">{(nguoiDung?.ho_ten || 'D').split(' ').map(t => t[0]).slice(-2).join('').toUpperCase()}</span>
                  </div>
                </div>
              ...
              <h3 className="mt-3 text-base md:text-lg font-bold text-slate-800">{nguoiDung?.ho_ten || 'Chủ hộ'}</h3>
              <p className="text-xs md:text-sm text-slate-500">Chủ hộ · {nguoiDung?.ten_thon || 'Xã Bà Nà'}</p>
            </div>
