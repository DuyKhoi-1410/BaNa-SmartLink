    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 max-h-[85vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-6 py-5">
          ...
          {hoDan.danhSachThayDoi && hoDan.danhSachThayDoi.length > 0 && (
            <div className="mb-3 px-3 py-2 bg-amber-50 border border-amber-200 rounded-lg">
              <p className="text-xs text-amber-700 font-medium">Hộ dân đã sửa {hoDan.danhSachThayDoi.length} chỉ tiêu, chỉ cần duyệt các mục đánh dấu "Đã sửa".</p>
            </div>
          )}
          ...
          <div className="space-y-2">
            {ctDanNhap.map(ct => {
              ...
              const daSua = hoDan.danhSachThayDoi ? hoDan.danhSachThayDoi.includes(ct) : true
              const giuNguyen = hoDan.danhSachThayDoi && !daSua
              return (
                <div key={ct} className={`rounded-xl border overflow-hidden ${giuNguyen ? 'border-slate-100 bg-slate-50/30 opacity-60' : daSua && hoDan.danhSachThayDoi ? 'border-amber-200 bg-amber-50/30' : 'border-slate-200 bg-slate-50/50'}`}>
                  <div className="flex items-center justify-between px-4 py-3">
                    <div className="flex-1 min-w-0 flex items-center gap-1.5 flex-wrap">
                      ...
                      {giuNguyen && <span className="text-[10px] font-medium px-1.5 py-0.5 rounded bg-slate-100 text-slate-400">Giữ nguyên</span>}
                      {daSua && hoDan.danhSachThayDoi && <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded bg-amber-100 text-amber-700">Đã sửa</span>}
                    </div>
                    <span className={`text-base font-bold ml-3 ${giuNguyen ? 'text-slate-400' : 'text-slate-800'}`}>
