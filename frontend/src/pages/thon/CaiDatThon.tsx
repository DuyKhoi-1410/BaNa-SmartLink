...
export default function CaiDatThon() {
  const { nguoiDung } = useAuth()
  const location = useLocation()
  const laTrangGoc = location.pathname === '/thon/cai-dat' || location.pathname === '/thon/cai-dat/'
        ...
        <div className={`${moSidebar ? 'block' : 'hidden'} md:block w-full md:w-72 md:flex-shrink-0`}>
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <div className="flex flex-col items-center -mt-10 md:-mt-12 pb-5 md:pb-6 px-5">
              <div className="relative">
                <div className="w-20 h-20 md:w-24 md:h-24 bg-white rounded-full border-4 border-white shadow-md flex items-center justify-center">
                  <div className="w-16 h-16 md:w-20 md:h-20 bg-gradient-to-br from-blue-400 to-indigo-500 rounded-full flex items-center justify-center">
                    <span className="text-white text-xl md:text-2xl font-bold">{(nguoiDung?.ho_ten || 'T').split(' ').map(t => t[0]).slice(-2).join('').toUpperCase()}</span>
                  </div>
                </div>
              ...
              <h3 className="mt-3 text-base md:text-lg font-bold text-slate-800">{nguoiDung?.ho_ten || 'Cán bộ thôn'}</h3>
              <p className="text-xs md:text-sm text-slate-500">Cán bộ {nguoiDung?.ten_thon || 'thôn'} · Xã Bà Nà</p>
            </div>
