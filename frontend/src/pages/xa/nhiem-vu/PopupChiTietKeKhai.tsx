...
export default function PopupChiTietKeKhai({ hienPopup, dongPopup, baoCao, danhSachThon, tongHopCT }) {
  useEffect(() => {
    if (!hienPopup) return
  ...
  if (!hienPopup || !baoCao) return null
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-[90vw] mx-4 max-h-[85vh] overflow-hidden flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
          <div>
            <h2 className="text-lg font-bold text-slate-800">Chi tiết kê khai</h2>
            <p className="text-xs text-slate-500 mt-0.5">{baoCao.ten}</p>
          </div>
                  ...
                  <tr className="bg-slate-50">
                    ...
                    {danhSachThon.map((thon, i) => (
                      <th key={i} className="text-center px-3 py-3 font-semibold text-xs uppercase tracking-wider border-b border-slate-200 min-w-[100px]">
                        <span className={thon.daNop ? 'text-slate-600' : 'text-slate-400'}>
                          {thon.ten.replace('Thôn ', '')}
                        </span>
                        {thon.daNop ? (
                        ...
                        <td key={i} className="text-center px-3 py-3 border-b border-slate-100">
                          {thon.daNop ? (
                            <span className="font-semibold text-slate-700">
                              {(thon.duLieuCT?.[ct] ?? 0).toLocaleString('vi-VN')}
                            </span>
                          ) : (
