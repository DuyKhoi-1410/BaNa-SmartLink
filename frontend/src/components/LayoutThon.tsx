...
import { BarChart3, ClipboardList, Bell, Settings, ChevronDown, User, LogOut, Search, X, BookOpen, Shield, FileText, HelpCircle, Menu } from 'lucide-react'
...
const duLieuTimKiem = [
  { nhom: 'Câu hỏi thường gặp', icon: HelpCircle, mau: 'text-blue-600 bg-blue-50', duongDan: '/thon/cai-dat/cau-hoi', danhSach: [
    { tieuDe: 'Làm sao duyệt dữ liệu kê khai của dân?', moTa: 'Vào mục "Nhiệm Vụ", chọn đợt kê khai, xem và duyệt từng hộ.' },
    { tieuDe: 'Làm sao xem tiến độ kê khai của dân?', moTa: 'Vào "Tổng Quan" để xem bảng tổng hợp tiến độ kê khai.' },
    { tieuDe: 'Nhập dữ liệu thủ công ở đâu?', moTa: 'Vào "Nhiệm Vụ", chọn đợt kê khai, bấm "Nhập thủ công" cho CT09, CT12, CT13, CT14.' },
    { tieuDe: 'Tôi quên mật khẩu thì phải làm sao?', moTa: 'Vào "Cài đặt" → "Thay đổi mật khẩu" hoặc liên hệ cán bộ xã.' },
    { tieuDe: 'Thông báo cho dân qua Zalo như thế nào?', moTa: 'Hệ thống tự động gửi thông báo qua Zalo OA khi có đợt kê khai mới.' },
    { tieuDe: 'Dữ liệu có được bảo mật không?', moTa: 'Có. Dữ liệu được mã hóa và lưu trữ an toàn trên hệ thống Supabase.' },
  ]},
  { nhom: 'Hướng dẫn sử dụng', icon: BookOpen, mau: 'text-blue-600 bg-blue-50', duongDan: '/thon/cai-dat/huong-dan', danhSach: [
    { tieuDe: 'Đăng nhập vào hệ thống', moTa: 'Mở trình duyệt, nhập tài khoản và mật khẩu cán bộ thôn đã được cấp.' },
    { tieuDe: 'Duyệt dữ liệu kê khai', moTa: 'Vào "Nhiệm Vụ" → chọn đợt → xem danh sách hộ đã kê khai → duyệt.' },
    { tieuDe: 'Nhập chỉ tiêu thủ công', moTa: 'Vào "Nhiệm Vụ" → chọn đợt → nhập CT09, CT12, CT13, CT14 của thôn.' },
    { tieuDe: 'Nộp báo cáo lên xã', moTa: 'Sau khi duyệt xong, bấm "Nộp lên xã" để gửi dữ liệu tổng hợp.' },
    { tieuDe: 'Gửi thông báo nhắc dân', moTa: 'Vào "Thông Báo" để tạo và gửi nhắc nhở cho các hộ chưa kê khai.' },
    { tieuDe: 'Quản lý tài khoản', moTa: 'Vào "Cài Đặt" để xem thông tin, đổi mật khẩu, quản lý thiết bị.' },
  ]},
  { nhom: 'Điều khoản sử dụng', icon: FileText, mau: 'text-purple-600 bg-purple-50', duongDan: '/thon/cai-dat/dieu-khoan', danhSach: [
    { tieuDe: 'Quyền và trách nhiệm cán bộ thôn', moTa: 'Duyệt dữ liệu kê khai, nhập chỉ tiêu thủ công, nộp báo cáo lên xã.' },
  ...
  { nhom: 'Chính sách quyền riêng tư', icon: Shield, mau: 'text-green-600 bg-green-50', duongDan: '/thon/cai-dat/dieu-khoan', danhSach: [
...
const menuCoDinh: any[] = [
  { ten: 'Tổng Quan', duongDan: '/thon', icon: BarChart3 },
  { ten: 'Nhiệm Vụ', duongDan: '/thon/nhiem-vu', icon: ClipboardList },
  { ten: 'Thông Báo', duongDan: '/thon/thong-bao', icon: Bell, laBadge: true },
  { ten: 'Cài Đặt', duongDan: '/thon/cai-dat', icon: Settings },
]

export default function LayoutThon() {
  const { nguoiDung, dangXuat } = useAuth()
  const soChuaDoc = useThongBaoChuaDoc(nguoiDung?.thon_id)
    ...
    <div className="min-h-screen flex flex-col bg-slate-50">
      ...
      <header className="sticky top-0 z-50 bg-blue-500 shadow-lg shadow-blue-500/20">
        <div className="max-w-7xl mx-auto px-4 md:px-6 flex items-center justify-between h-14 md:h-16">
          {/* Logo + Tên */}
          <div className="flex items-center gap-2 md:gap-3 cursor-pointer" onClick={() => navigate('/thon')}>
          ...
          <div className="hidden lg:block relative">
            ...
            >
              ...
              <span className="text-sm font-medium">{nguoiDung?.ho_ten || 'Cán bộ thôn'}</span>
            ...
            {moDropdown && (
              <>
                ...
                <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-xl shadow-xl border border-slate-200 py-2 z-50">
                  <button
                    onClick={() => { setMoDropdown(false); navigate('/thon/cai-dat/thong-tin') }}
          ...
          <div className="lg:hidden border-t border-white/20">
            <div className="max-w-7xl mx-auto px-4 py-3 space-y-1">
              ...
              <div className="border-t border-white/20 mt-2 pt-2 space-y-1">
                <button
                  onClick={() => { navigate('/thon/cai-dat/thong-tin'); setMoMenu(false) }}
